...
describe("runMigrations", () => {
  ...
  it("runs V1 and V2 when at version 0", () => {
    ...
    // Sets version to 2
    expect(calls.some((sql: string) => sql.includes("PRAGMA user_version = 2"))).toBe(true);
  });

  it("runs V2 when at version 1", () => {
    ...
    expect(calls.some((sql: string) => sql.includes("DROP TRIGGER IF EXISTS releases_ad"))).toBe(true);
    expect(calls.some((sql: string) =>
      sql.includes("CREATE TRIGGER releases_au") && sql.includes("json_each(old.artists)")
    )).toBe(true);
  ...
  it("skips all migrations when already at version 2", () => {
